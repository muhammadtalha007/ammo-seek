...
@section('content')
    ...
    <div class="px-5">
        <div class="container">
            <h5 class="mt-4 mb-3">Add Caliber</h5>
            @if($errors->any())
                <div class="alert alert-danger">
                    <h4>{{$errors->first()}}</h4>
                </div>
            @endif
            <form method="post" action="{{url("/save-caliber-data")}}" onsubmit="return onSubmitForm();">
                {{csrf_field()}}
                <div class="form-group">
                    <label>Name:</label>
                    <input type="text" class="form-control col-lg-5 col-md-5 col-sm-5" placeholder="Enter Caliber"
                           name="name"
                           id="name">
